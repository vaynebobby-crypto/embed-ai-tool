...
- 通过 VISA 资源管理器自动列出已连接的仪器。
- 支持 pyvisa-py（纯 Python 后端）和 NI-VISA 后端。
- 若探测失败，提示安装 `pyvisa` / `pyvisa-py` 或 NI-VISA 驱动。
...
- 跨平台（Windows / Linux / macOS），依赖 `pyvisa` 库。
- Windows：推荐 NI-VISA 或 pyvisa-py 纯 Python 后端。
- Linux / macOS：使用 pyvisa-py 后端，无需安装厂商驱动。
